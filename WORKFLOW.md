...
- `scripts/validate-capabilities.ps1` and `rebuild-capability-index.ps1`: maintainer-side upkeep of the independent `capabilities/` library, run by CI rather than during product work. The first checks every `capabilities/recipes/*.json` default-deny (fields, status and strength enums, payload hash, unsigned acknowledgement, privacy) and that `capabilities/INDEX.json` still matches the recipes on disk; the second regenerates that index. Both build on `scripts/lib/capability-common.ps1`.
- The runtime half of that library -- the gap classifier, the read-only capability-to-tool bridge, and the machine-local learned-tool recorder -- is deliberately not named in this document, and neither is the regression that guards it (the `Run capability wiring regression` CI step). `scripts/start-here.ps1` prints the classifier command at the moment the six analysis categories are still undecided, and prints the learned-tool recorder command on the EXE analysis track when a tool role is still empty (the tool inventory is EXE-analysis-centric, so a source-reuse product, which builds from its own source, does not surface it); that regression asserts the executor really prints it there. `validate-skill-layout.ps1` derives reachability from the script filenames written into this document and then follows those scripts' own text, so naming any of them here -- the regression included, since it necessarily quotes the classifier -- would mark the whole library reachable on prose alone. That is the green that let it ship unreachable once already. Leave the filenames out: the wiring is what has to keep them reachable.
